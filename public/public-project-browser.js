(() => {
  ...
  function showPreview() {
    ...
    preview.querySelector("button").addEventListener("click", () => {
      ...
      if (!saved.some((p) => p.id === id)) { const m = budget(selected); saved.unshift({id, title:title(selected), owner:agency(selected), category:"Public ICT reference", objective:"Improve Customer Experience", duration:[selected["Project start date"],selected["Project end date"]].filter(Boolean).join(" – ") || "Not reported", cost:m, staff:0, status:"Submitted", summary:selected["Project description"] || "Public project reference.", benefits:"Define the expected benefits for this organisation before review.", risks:"Source confidence: " + (selected["DCA 2026"] || "not reported") + ". Add project-specific risks before review.", scores:{}, rationales:{}, missing:["Staff requirement","Project-specific risks","Expected benefits for this organisation"], isCustom:true}); localStorage.setItem(storageKey, JSON.stringify(saved)); }
